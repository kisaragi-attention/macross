...
from utils import should_omit_packet, read_pcap, PREFIX_EV_STA_ID, PREFIX_TO_ATK_ID, PREFIX_TO_SCENARIO_ID, PREFIX_TO_ATTACK_ID
...
def transform_pcap(path, output_path: Path = None, output_batch_size=10000):
    ...
    for i, packet in enumerate(read_pcap(path)):
        ...
        if arr is not None:
            ...
            status_label = PREFIX_EV_STA_ID.get(prefix)
            binary_label = PREFIX_TO_ATK_ID.get(prefix)
            scenario_label = PREFIX_TO_SCENARIO_ID.get(prefix)
            attack_label = PREFIX_TO_ATTACK_ID.get(prefix)
            row = {
